...
test('Docker healthcheck uses the IPv4 nginx listener so the injected token is applied', () => {
  const dockerfile = readProjectFile('Dockerfile');

  assert.match(dockerfile, /HEALTHCHECK[\s\S]*wget -qO- http:\/\/127\.0\.0\.1:8080\/api\/health/);
  assert.doesNotMatch(dockerfile, /wget -qO- http:\/\/localhost:8080\/api\/health/);
});
