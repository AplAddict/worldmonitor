...
function credentialSyncToken() {
  const tokenFile = process.env[CREDENTIAL_SYNC_TOKEN_FILE_ENV];
  if (!tokenFile) return '';
  try { return readFileSync(tokenFile, 'utf8').trim(); } catch { return ''; }
}

function credentialSyncAuthorized(req) {
  const expected = credentialSyncToken();
  const provided = req.headers[CREDENTIAL_SYNC_HEADER] || '';
...
async function dispatch(requestUrl, req, routes, context) {
  ...
  if (requestUrl.pathname === '/api/self-hosted/credential-sync') {
    if (req.method !== 'POST') return json({ error: 'POST required' }, 405);
    if (!credentialSyncAuthorized(req)) return json({ error: 'Forbidden' }, 403);
    const body = await readBody(req);
    try {
      const { entries } = JSON.parse(body?.toString('utf8') || '{}');
      if (!Array.isArray(entries) || entries.length < 1 || entries.length > CREDENTIAL_SYNC_KEYS.size) throw new Error('invalid entries');
      for (const entry of entries) {
        if (!CREDENTIAL_SYNC_KEYS.has(entry?.key) || typeof entry.value !== 'string' || !entry.value.length || entry.value.length > 4096) {
      ...
      return json({ ok: true, updated: entries.map(({ key }) => key) });
    } catch {
